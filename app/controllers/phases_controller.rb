class PhasesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xls
      format.pdf {
        pdf=PhasePdf.new(current_project.phases,view_context)
        send_data  pdf.render, filename: "phases.pdf",
...
def rake_phases
    ...
    if params[:file].nil?
      ...
      if !params[:file].nil? && @errors.empty?
        doc.xpath("//phase").each do |phase|
          ...
          flash[:notice]="Phases successfully added to the project"
          end
      end
    end
  ...
  def make
    params[:phases].each do |p|
      ...
      @default_phase=Defaultphase.find(:first,:conditions=>['name=?',p])
      if !@default_phase.parent_id.nil?
        @default_phase_name=Defaultphase.find_by_id(@default_phase.parent_id).name 
        if !@default_phase_name.nil?
            @parent_phase=Phase.find_or_create_by_name_and_project_id(:name=>@default_phase_name,:project_id=>current_project.id,:start=>"2012-02-10 00:00:00.000000",:end=>"2012-04-10 00:00:00.000000")
            @phase=Phase.find_or_create_by_name_and_project_id_and_parent_id(:project_id=>current_project.id, :name=>p, :parent_id=>@parent_phase.id,:start=>"2012-02-10 00:00:00.000000",:end=>"2012-04-10 00:00:00.000000")
        end 
     else
       
        @phase=Phase.find_or_create_by_name_and_project_id(:project_id=>current_project.id, :name=>p,:start=>"2012-02-10 00:00:00.000000",:end=>"2012-04-10 00:00:00.000000")
        @phase.save
     end 
   end
